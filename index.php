...
<body style="background-color: #33CCFF">
	<div class="ui container">
		<div class="ui segment" style="max-width: 800px; margin: 0 auto;">
			<h2 class="ui header center">Azure N-Tier Architecture</h2>
...
<script type="text/javascript">
	...
	$('.setting').click(() => {
		...
		while (kv_name.length === 0) 
			kv_name = prompt('Enter keyvault name');
	...
	function get_data() {
		$.ajax({
			url: `data.php?operation=get&read_db_key=${read_key}&write_db_key=${write_key}&kv_name=${kv_name}`,
			success: res => {
				if (res.error) {
					alert(res.message);
					return;
	...
	$('form').submit(event => {
		...
		$.ajax({
			url: `data.php?operation=add&product_name=${$('#new_product_name').val()}&product_quantity=${$('#new_product_quantity').val()}&product_price=${$('#new_product_price').val()}&read_db_key=${read_key}&write_db_key=${write_key}&kv_name=${kv_name}`,
			success: res => {
				if (res.error) {
					alert(res.message);
					return;
	...
	$(document).on('click', '.edit.icon', function() {
		...
		$.ajax({
			url: `data.php?operation=update&product_id=${product_id}&product_name=${product_name}&product_quantity=${product_quantity}&product_price=${product_price}&read_db_key=${read_key}&write_db_key=${write_key}&kv_name=${kv_name}`,
			success: res => {
				if (res.error) {
					alert(res.message);
					return;
	...
	$(document).on('click', '.trash.icon', function() {
		...
		$.ajax({
			url: `data.php?operation=delete&product_id=${product_id}&read_db_key=${read_key}&write_db_key=${write_key}&kv_name=${kv_name}`,
			success: res => {
				if (res.error) {
					alert(res.message);
					return;
